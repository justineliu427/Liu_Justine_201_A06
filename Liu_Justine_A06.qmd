...
[https://github.com/justineliu427/Liu_Justine_201_A06.gitl](https://github.com/justineliu427/Liu_Justine_201_A06.git)
...
```{r}
# add the following values into anew list called height
...
# iterate through the names of each player to calculate the points per game for each player 
...
```{r}
mat_rounded <- apply(mat_rand, function(x){

  # convert digits into characters so that it's countable
...
temp_vals <- max_temps[, -c(1, 2)]

# apply the mean function to the rest of the columns
avg_temp <- apply(temp_vals, 2, mean)
print(avg_temp)
```
...
```{r}
# neglect the first two columns
temp_vals <- co_temps[, -c(1,2)]

#take median of each row (year)
median_annual<-apply(temp_vals, 1, median)

# find the year indices with the maximum and minimum median temps 
max_year_index <- which.max(median_annual)
min_year_index <- which.min(median_annual)

# find the year at the indices
max_year <- co_temps[max_year_index, "year"]
min_year <- co_temps[min_year_index, "year"]

print(paste0("The maximum median annual temperature of ", median_annual[max_year_index], " occurred in ", max_year, " and the minimum median annual temperature of ", median_annual[min_year_index], " occurred in ", min_year, "."))
```
...
digit_sum <- function(n){
  ...
  # return the digit if it is only single digit 
  if (n < 10) {
    return(n)
  } else {
    ...
    # grabs the remaining digits
    remaining <- n %/% 10

    # adds the last digit with the remaining digits
